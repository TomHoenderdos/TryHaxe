...
Editor.prototype = {
	...
	,makePath: function(link) {
		var parts = link.split("/");
		var path = Lambda.array(Lambda.map(parts,Editor.normalize));
		if(path[0] == "") {
			path.shift();
	...
	,code: function(t,style) {
		...
		case "haxe":
			...
			break;
		case "raw":
			if(this.config.allowRaw) return StringTools.htmlUnescape(t);
			break;
		default:
		}
		return "<pre" + cl + ">" + t + "</pre>";
	}
	,makeTitle: function(titles,code,str) {
		var part = new EReg("[^a-z0-9]+","g").replace(str.toLowerCase(),"-");
		while(part.charCodeAt(0) == 45) part = part.substr(1);
		while(part.length > 0 && part.charCodeAt(part.length - 1) == 45) part = part.substr(0,part.length - 1);
		if(titles.exists(part)) {
			var count = 2;
			while(titles.exists(part + "_" + count)) count++;
			part += "_" + count;
		}
	...
	,paragraph: function(t) {
		...
		var titles = new Hash();
		titles.set("",true);
		t = new EReg("====== ?(.*?) ?======","g").customReplace(t,function(r) {
			return me.makeTitle(titles,"h1",r.matched(1));
		});
		t = new EReg("===== ?(.*?) ?=====","g").customReplace(t,function(r) {
			return me.makeTitle(titles,"h2",r.matched(1));
		});
		t = new EReg("==== ?(.*?) ?====","g").customReplace(t,function(r) {
			return me.makeTitle(titles,"h3",r.matched(1));
		});
		...
		t = new EReg("\\[\\[([^\\]]*?)\\]\\]","").customReplace(t,function(r) {
			...
			if(link.substr(link.length - 2,2) == "/*") {
				var path = me1.makePath(link.substr(0,link.length - 2)).path;
				var list = me1.subcache.get(path.join("/"));
				if(list == null) {
					list = me1.getSubLinks(path);
					me1.subcache.set(path.join("/"),list);
				}
			...
			var i = me1.getInfos(r.matched(1));
	...
	,format: function(t) {
		...
		var div_begin = new EReg("^\\[(/?[A-Za-z0-9_ ]+)\\]","");
		var div_end = new EReg("\\[(/?[A-Za-z0-9_ ]+)\\]$","");
		...
		while(_g < _g1.length) {
			...
			switch(p.substr(0,3)) {
			case "":case "<h1":case "<h2":case "<h3":case "<ul":case "<pr":case "##C":case "<sp":
				b.b[b.b.length] = p == null?"null":p;
				break;
			default:
				b.b[b.b.length] = "<p>";
				b.b[b.b.length] = p == null?"null":p;
			...
			b.b[b.b.length] = after == null?"null":after;
		}
		var _g = 0;
		while(_g < pstack.length) {
			var d = pstack[_g];
			++_g;
			b.b[b.b.length] = "</div>";
		}
...
haxe.TimerQueue = $hxClasses["haxe.TimerQueue"] = function(delay) {
	this.delay = delay == null?1:delay;
	this.q = new Array();
};
haxe.TimerQueue.__name__ = ["haxe","TimerQueue"];
haxe.TimerQueue.prototype = {
	delay: null
	,t: null
	,q: null
	,add: function(f) {
		...
		if(this.t == null) {
			...
			this.t.run = this.process.$bind(this);
		}
	}
	,process: function() {
		var f = this.q.shift();
...
var JsTools = $hxClasses["JsTools"] = function() { }
